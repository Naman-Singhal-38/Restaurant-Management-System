...
void delivery_order_status(int order_num)
{
    ...
    {
        printf("delivery boy name: %s\n",delivery_boy[deliveryorder[(order_num-1)].delivery_boy_index].name);
        printf("delivery boy name: %s\n",delivery_boy[deliveryorder[(order_num-1)].delivery_boy_index].number);                          
...
void dispatch_delivery_order(int order_num)
{
    if(strcmp( deliveryorder[(order_num-1)].delivery_status ,"preparing order")==0)
    {
        ...
        strcpy(delivery_boy[index].delivery_boy_status,"unavailable");
...
int main()
{
    int order_no,num,size,i,y,e=1,f=1,g=1;
                ...
                while(e)
                {
                    ...
                    case 10:
                        for(i=0;i<delivery_order_num;i++)
                        {
                            ...
                            printf("delivery boy name: %s\n",delivery_boy[deliveryorder[i].delivery_boy_index].name);
                            printf("delivery boy name: %s\n",delivery_boy[deliveryorder[i].delivery_boy_index].number);
                            printf("DELIVERY STATUS: %s\n",deliveryorder[i].delivery_status);
                            printf("\n\n");
                        }
                        break;
                    case 11:
                        y= delivery_boy_available();
